...
# Task 1.4's own boundary: a submitted PR may touch only the permitted load-harness file
# plus the answer sheet. Keep this in sync with submission_validation.ALLOWED_PATHS.
LOAD_HARNESS_DIFF_ALLOWLIST = frozenset({"loadtest/model_provider_latency.py", "submission.yaml"})
HEADER_FIELDS = (
    "File:",
    "Component:",
    "Purpose:",
...
def test_load_harness_change_stays_within_the_permitted_diff() -> None:
    """Catch a submitted change that reaches outside Task 1.4's load-harness boundary."""
    changed = set(_changed_paths(TASK_ROOT))
    assert changed <= LOAD_HARNESS_DIFF_ALLOWLIST
